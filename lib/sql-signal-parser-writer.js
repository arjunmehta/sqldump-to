...
const { createFileWriteStream } = require('./util/fs');
...
class SQLSignalParserWriter {
  ...
  setCurrentTableQueue() {
    ...
    if (databaseQueue[tableName] === undefined) {
      ...
      if (this.isWritingToFile === true) {
        ...
        writeStream = createFileWriteStream({ directory, filename });
      }
  ...
  handleData(data, cb = () => {}) {
    // console.log({ data });
    const msg = data.replace(/(?!\B'[^']*)(NULL|\\N)(?![^']*'\B)/g, 'null');
  ...
  writeFromBuffers() {
    ...
    Object.keys(databaseWriteQueues).forEach((database) => {
      ...
      Object.keys(databaseQueue).forEach((tableName) => {
        const { buffer, writeStream } = databaseQueue[tableName];
