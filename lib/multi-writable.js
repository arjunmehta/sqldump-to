...
class MultiWriteable extends stream.Writable {
  ...
  runReadSchema() {
    ...
    if (indexOfNextParen === -1) {
      ...
      if (contents) {
        ...
        console.log('Detected schema', JSON.stringify(schema, null, 2));

        this.cleanBuffer();
        this.state = null;

        // Done processing Create Table command. Jump to end of line
  ...
  getNextCommandParenSet() {
    ...
    while (
      shouldContinue === true
      && this.bufferPosition < this.buffer.length - 1
    ) {
