...
class MultiWriteable extends stream.Writable {
  ...
  processQueueItem(queueItem) {
    const probablyCleanRow = queueItem;
    const hasEnd = probablyCleanRow.indexOf(this.endDelimeter) > -1;
    const hasBeg = probablyCleanRow.indexOf(this.beginDelimeter) > -1;
    let rowString = '';

    // not clean, has trailing );
    if (hasEnd) {
      const splitChunk = probablyCleanRow.split(this.endDelimeter);
      const [firstPart] = splitChunk;
      rowString = `[${firstPart}]`;
    }

    // not clean, has leading INSERT INTO __ VALUES (...
    if (hasBeg) {
      const splitChunk = probablyCleanRow.split(this.beginDelimeter);
      const [beginning, secondPart] = splitChunk;
    ...
    return rowString || `[${probablyCleanRow}]`;
  }
}
