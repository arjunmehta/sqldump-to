...
const BRACKET_CLOSE_OCT = BRACKET_CLOSE_BUFF[0];
const BRACKET_OPEN_OCT = BRACKET_OPEN_BUFF[0];
const NEWLINE_OCT = NEWLINE_BUFF[0];
...
class SQLConverterStream extends stream.Writable {
  ...
  continueInsert() {
    ...
    while (
      ...
      bufferLength = queueItem.length + 3;
      rowBuffer = Buffer.allocUnsafe(bufferLength);

      queueItem.copy(rowBuffer, 1);

      rowBuffer[0] = BRACKET_OPEN_OCT;
      rowBuffer[bufferLength - 2] = BRACKET_CLOSE_OCT;
      rowBuffer[bufferLength - 1] = NEWLINE_OCT;

      worker = readyWorkers.pop();
      writes.push(this.writeWorkers.writeToNextAvailableWorker(worker, rowBuffer));
