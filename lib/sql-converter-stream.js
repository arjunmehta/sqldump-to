...
class SQLConverterStream extends stream.Writable {
  ...
  switchCurrentTable(tableName) {
    if (this.currentDatabase.tables[tableName]) {
      ...
      this.writeWorkers.sendSchema(this.currentDatabase.tables[tableName]);
    } else {
  ...
  continueInsert() {
    ...
    let writeRow = new Promise((resolve) => { resolve(); });

    if (queueItem && queueItem !== COMMAND_EXEC_BUFF) {
      const worker = this.writeWorkers.readyWorkers.pop();
      writeRow = this.writeWorkers.writeToNextAvailableWorker(worker, queueItem);
    }
