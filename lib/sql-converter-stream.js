...
class SQLConverterStream extends stream.Writable {
  ...
  get databaseName() {
    const databaseName = this.currentDatabase || 'default';
    return databaseName;
  }
  ...
  setTable(tableName) {
    if (this.databases[this.databaseName].tables[tableName]) {
  ...
  continueInsert() {
    ...
    while (
      ...
      writes.push(this.writeWorkers.writeToNextAvailableWorker(worker, queueItem));
  ...
  runUseDatabase() {
    ...
    if (contents === undefined) {
      ...
      this.setDatabase(cleanVarName(contents));
      this.runSeekEOL();
    }
  }
  ...
  runReadSchema() {
    ...
    if (tableString === undefined) {
      ...
      if (contents) {
        ...
        const schema = {
          databaseName: this.currentDatabase,
          tableName,
          columns,
        };

        this.databases[this.databaseName].tables[tableName] = schema;
